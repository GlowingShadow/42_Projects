...
#finlande
   ...
   0x080484d5 <+81>:    mov    %ecx,0xc(%eax)				; 12
   0x080484d8 <+84>:    movzwl 0x10(%edx),%ecx				; 16
   0x080484dc <+88>:    mov    %cx,0x10(%eax)				; 16
   0x080484e0 <+92>:    movzbl 0x12(%edx),%edx				; 18
   0x080484e4 <+96>:    mov    %dl,0x12(%eax)				; 18
   0x080484e7 <+99>:    jmp    0x804850a <greetuser+134>
# nl
   0x080484e9 <+101>:   mov    $0x804872a,%edx
   0x080484ee <+106>:   lea    -0x48(%ebp),%eax				; -72
   ...
   0x08048505 <+129>:   mov    %dx,0xc(%eax)				; low register ?
   0x08048509 <+133>:   nop
...
Dump of assembler code for function main:
   ...
   0x08048538 <+15>:    cmpl   $0x3,0x8(%ebp)				; <= 3ac, 2 args
   0x0804853c <+19>:    je     0x8048548 <main+31>
   0x0804853e <+21>:    mov    $0x1,%eax
   0x08048543 <+26>:    jmp    0x8048630 <main+263>
   0x08048548 <+31>:    lea    0x50(%esp),%ebx				; 80 buffer ?
   0x0804854c <+35>:    mov    $0x0,%eax
   0x08048551 <+40>:    mov    $0x13,%edx					; 19
   0x08048556 <+45>:    mov    %ebx,%edi
   0x08048558 <+47>:    mov    %edx,%ecx
   0x0804855a <+49>:    rep stos %eax,%es:(%edi)			; while (edx != 0) // *$edi = 0xbffff8f5 =  "/home/user/bonus2/bonus2"
   0x0804855c <+51>:    mov    0xc(%ebp),%eax				; 12
   0x0804855f <+54>:    add    $0x4,%eax
   0x08048562 <+57>:    mov    (%eax),%eax
   0x08048564 <+59>:    movl   $0x28,0x8(%esp)				; 40	0xbffff688
   0x0804856c <+67>:    mov    %eax,0x4(%esp)				;		0xbffff684: 0xbffff90e
   0x08048570 <+71>:    lea    0x50(%esp),%eax				; 80	0xbffff6d0
   0x08048574 <+75>:    mov    %eax,(%esp)
   0x08048577 <+78>:    call   0x80483c0 <strncpy@plt>		; call strncpy(buf, argv[1], 40)
   0x0804857c <+83>:    mov    0xc(%ebp),%eax				; 12
   ...
   0x0804859a <+113>:   call   0x80483c0 <strncpy@plt>		; call strncpy(buf, argv[2], 32)
   0x0804859f <+118>:   movl   $0x8048738,(%esp)
   0x080485a6 <+125>:   call   0x8048380 <getenv@plt>		; call getenv
   0x080485ab <+130>:   mov    %eax,0x9c(%esp)				; 156	0xbffff71c : 0xbfffff2e (getenv ret)
   0x080485b2 <+137>:   cmpl   $0x0,0x9c(%esp)				; if ("")
   0x080485ba <+145>:   je     0x8048618 <main+239>
   0x080485bc <+147>:   movl   $0x2,0x8(%esp)
   0x080485c4 <+155>:   movl   $0x804873d,0x4(%esp)
   0x080485cc <+163>:   mov    0x9c(%esp),%eax				; 156
   0x080485d3 <+170>:   mov    %eax,(%esp)
   0x080485d6 <+173>:   call   0x8048360 <memcmp@plt>		; call memcmp  if ("fi")
   ...
   0x080485fb <+210>:   mov    0x9c(%esp),%eax				; 156
   0x08048602 <+217>:   mov    %eax,(%esp)
   0x08048605 <+220>:   call   0x8048360 <memcmp@plt>		; call memcmp  if ("nl")
   0x0804860a <+225>:   test   %eax,%eax
   0x0804860c <+227>:   jne    0x8048618 <main+239>
   0x0804860e <+229>:   movl   $0x2,0x8049988				; ; <language> = 2
   0x08048618 <+239>:   mov    %esp,%edx
   0x0804861a <+241>:   lea    0x50(%esp),%ebx				; 80
   ...
   0x08048630 <+263>:   lea    -0xc(%ebp),%esp				; -12
...
Hello
Goedemiddag!	; dutch for "good afternoon"
LANG
