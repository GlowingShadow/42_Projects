...
Glfw::Glfw(unsigned int width, unsigned int height) {
	this->_width = min(width, MAX_WIDTH);
	this->_width = max(width, MIN_WIDTH);
	this->_height = min(height, MAX_HEIGHT);
	this->_height = max(height, MIN_HEIGHT);
	this->_title = DEFAULT_TITLE;
	this->init();
}
...
void	Glfw::setMouseAngle(double angle) {// set to negative to deactivate
	this->_mouseAngle = min(angle, MOUSE_MAX_ANGLE);
