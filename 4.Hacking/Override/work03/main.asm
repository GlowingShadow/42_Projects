Dump of assembler code for function decrypt:
   ...
   0x08048668 <+8>:     mov    %gs:0x14,%eax				; 20
   0x0804866e <+14>:    mov    %eax,-0xc(%ebp)				; -12
   0x08048671 <+17>:    xor    %eax,%eax
   0x08048673 <+19>:    movl   $0x757c7d51,-0x1d(%ebp)		; -29		Q}|u
   0x0804867a <+26>:    movl   $0x67667360,-0x19(%ebp)		; -25		`sfg
   0x08048681 <+33>:    movl   $0x7b66737e,-0x15(%ebp)		; -21		~sf{
   0x08048688 <+40>:    movl   $0x33617c7d,-0x11(%ebp)		; -17		}|a3
   0x0804868f <+47>:    movb   $0x0,-0xd(%ebp)				; -13		'\0'
   0x08048693 <+51>:    push   %eax
   0x08048694 <+52>:    xor    %eax,%eax
   0x08048696 <+54>:    je     0x804869b <decrypt+59>
   0x08048698 <+56>:    add    $0x4,%esp					; add 4 to ?
   0x0804869b <+59>:    pop    %eax
   0x0804869c <+60>:    lea    -0x1d(%ebp),%eax				; -29
   0x0804869f <+63>:    movl   $0xffffffff,-0x2c(%ebp)		; -44
   ...
   0x080486bb <+91>:    mov    %eax,-0x24(%ebp)				; -36
   0x080486be <+94>:    movl   $0x0,-0x28(%ebp)				; -40
   0x080486c5 <+101>:   jmp    0x80486e5 <decrypt+133>

   0x080486c7 <+103>:   lea    -0x1d(%ebp),%eax				; -29
   0x080486ca <+106>:   add    -0x28(%ebp),%eax				; -40
   ...
   0x080486d9 <+121>:   lea    -0x1d(%ebp),%eax				; -29
   0x080486dc <+124>:   add    -0x28(%ebp),%eax				; -40
   0x080486df <+127>:   mov    %dl,(%eax)
   0x080486e1 <+129>:   addl   $0x1,-0x28(%ebp)				; -40
   0x080486e5 <+133>:   mov    -0x28(%ebp),%eax				; -40
   0x080486e8 <+136>:   cmp    -0x24(%ebp),%eax				; -36
   0x080486eb <+139>:   jb     0x80486c7 <decrypt+103>

   0x080486ed <+141>:   lea    -0x1d(%ebp),%eax				; -29
   ...
   0x0804872f <+207>:   mov    -0xc(%ebp),%esi				; -12
   0x08048732 <+210>:   xor    %gs:0x14,%esi				; 20
   0x08048739 <+217>:   je     0x8048740 <decrypt+224>
   0x0804873b <+219>:   call   0x80484c0 <__stack_chk_fail@plt>
   0x08048740 <+224>:   add    $0x40,%esp					; 64
...
Dump of assembler code for function test:
   ...
   0x0804874d <+6>:     mov    0x8(%ebp),%eax				; notre pass input
   0x08048750 <+9>:     mov    0xc(%ebp),%edx				; 0x1337d00d	322424845
   0x08048753 <+12>:    mov    %edx,%ecx
   0x08048755 <+14>:    sub    %eax,%ecx
   0x08048757 <+16>:    mov    %ecx,%eax
   0x08048759 <+18>:    mov    %eax,-0xc(%ebp)				; stock eax dans ebp-12
   0x0804875c <+21>:    cmpl   $0x15,-0xc(%ebp)				; 21
   0x08048760 <+25>:    ja     0x804884a <test+259>			; above go to rand()
   0x08048766 <+31>:    mov    -0xc(%ebp),%eax				; stock ebp-12 dans eax
   0x08048769 <+34>:    shl    $0x2,%eax					; mult * 4
   0x0804876c <+37>:    add    $0x80489f0,%eax
   0x08048771 <+42>:    mov    (%eax),%eax
   0x08048773 <+44>:    jmp    *%eax						; jump a *0x80489f0 +- 80
